...
def check_and_generate_file(path, content, max):
    ...
    if content_length > max:
        print(f"{path} is too long and greater than {max} characters: {content_length}")
    generate_file(path, content)

def read_sheet_data(sheet_id):
    # https://developers.google.com/chart/interactive/docs/spreadsheets
    # https://developers.google.com/chart/interactive/docs/querylanguage
    sheet_url = f"http://docs.google.com/spreadsheets/d/{sheet_id}/gviz/tq?&gid=0&headers=0"
    print(f"sheet_url = {sheet_url}")
    request = urllib.request.urlopen(sheet_url)
...
def populate_ios_metadata(data, metadata_dir):
    for language_code, language_name in IOS_LANGUAGES_CODES.items():            
...
def populate_metadata(sheet_id, prj_dir, customized_dir, platform):
    print(f"populate metadata: sheet_id = {sheet_id} prj_dir = {prj_dir} customized_dir = {customized_dir} platform = {platform}")
...
def copy_image_or_convert_if_possible(src_path, dst_path):
    if src_path.endswith(".png"):
        ...
        print(f"Convert {src_path} to {new_dst_path}")
        ...
        print(f"Ignore {src_path}")

# populate screenshots, TODO: code for android
def populate_screenshots(screenshots_input_dir, prj_dir, customized_dir, platform):
    print(f"populate screenshots: screenshots_input_dir = {screenshots_input_dir} prj_dir = {prj_dir} customized_dir = {customized_dir} platform = {platform}")
    ...
    for key, value in IOS_LANGUAGES_CODES.items():
